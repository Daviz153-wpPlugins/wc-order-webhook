...
class WCMW_Webhook {
    ...
    public static function send(int $order_id): void {
        ...
        $sent_products = $order->get_meta('_wcmw_sent_products') ?: [];
        $updated       = false;
        ...
        foreach ($order->get_items() as $item) {
            ...
            if (in_array($product_id, $sent_products, true)) continue;
    ...
    public static function test_send(): array {
        ...
        if (empty($url)) {
            return ['success' => false, 'message' => '테스트 URL이 설정되지 않았습니다.'];
        }
        ...
        $code = wp_remote_retrieve_response_code($response);
        if ($code >= 200 && $code < 300) {
            return ['success' => true, 'message' => "발송 성공 (HTTP {$code})"];
        }
        return ['success' => false, 'message' => "발송 실패 (HTTP {$code})"];
    }
}
